...
class Cancer 0.01 {
    ...
    field $x : param = 0;
    field $y : param = 0;
    field $width : param //= _get_terminal_dimensions($fh_out)->[0]    # TODO: 80 if DUMB terminal
        ;
    field $height : param //= _get_terminal_dimensions($fh_out)->[1]    # TODO: 25 if DUMB terminal
        ;
    ...
    sub is_terminal() {
        ...
        ( !!$term->getattr( fileno $fh_out ) ) ? $term : Cancer::Error->new( message => 'Failed to grab terminal info' );
    }

    sub get_state($fd) {
        my $term = is_terminal($fd);
        return $term unless $term;
        Cancer::Term::State->new( termios => $term );
    }

    sub make_raw($fd) {
        my $term = is_terminal($fd);
        return $term unless $term;
        my $old_state = Cancer::Term::State->new( termios => $term );
        my $t2        = is_terminal($fd);                               # 2nd copy that we can modify

        # Taken from Go's term/term_unix.go
        $t2->setiflag( $t2->getiflag() ^ ( IGNBRK | BRKINT | PARMRK | ISTRIP | INLCR | IGNCR | ICRNL | IXON ) );
        $t2->setoflag( $t2->getoflag() ^ (OPOST) );
        $t2->setlflag( $t2->getlflag() ^ ( ECHO | ECHONL | ICANON | ISIG | IEXTEN ) );
        $t2->setcflag( $t2->getcflag() ^ ( CSIZE | PARENB ) | CS8 );
        $t2->setcc( VMIN,  1 );
        $t2->setcc( VTIME, 0 );
        $t2->setattr( fileno $fd, TCSANOW ) ? $old_state : Cancer::Term::Error->new( message => 'Failed to set raw state' );
    }

    sub restore( $fd, $state ) {
        my $term = is_terminal($fd);
        return $term unless $term;
        #
        $state->termios->setattr( fileno $fd, TCSANOW ) ? !!1 : Cancer::Term::Error->new( message => 'Failed to restrore state' );
    }

    method height ( $h = () ) {
        $width      = $h               if defined $h;
        $max_height = $min_height = $h if defined $h && $h > $max_height;
        $height;
    }

    method width ( $w = () ) {
        $width     = $w              if defined $w;
        $max_width = $min_width = $w if defined $w && $w > $max_width;
        $width;
    }

    method size ( $w = (), $h = () ) {
        $w = $h = undef unless defined $w && defined $h;
...
=cut
    ...
    method read ( $length //= 1024 ) {
        ...
        sysread $fh_in, my ($ret), $length;
        return $ret;
    }

    method read_line() {
        scalar readline $fh_in;

        #~ warn +readline $i;
    }

    method read_password() {
        my $t1 = is_terminal($fh_in);    # 2nd copy that we can modify
        my $t2 = is_terminal($fh_in);    # 2nd copy that we can modify
        ...
        $t2->setcc( VMIN,  1 );
        $t2->setcc( VTIME, 0 );
        $t2->setattr( fileno $fh_out, TCSANOW ) ? 1 : return Cancer::Term::Error->new( message => 'Failed to set password state' );
        my $bytes = scalar readline $fh_in;
        $t1->setattr( fileno $fh_out, TCSANOW ) ? 1 : return Cancer::Term::Error->new( message => 'Failed to reset state' );
        return $bytes;
    }
    #
