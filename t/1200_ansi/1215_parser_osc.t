...
subtest 'TestOscSequence' => sub {
    my @dispatched;

    # -- parse: \e]2;charmbracelet: ~/Source/bubbletea\a
    @dispatched = ();
    my $p = make_parser();
    set_handler( $p,
    ...
    # -- bell_terminated: \e]11;ff/00/ff\a
    @dispatched = ();
    $p          = make_parser();
    set_handler( $p, { HandleOsc => sub ( $cmd, $data ) { push @dispatched, $data }, } );
    ...
    # -- esc_st_terminated: \e]11;ff/00/ff\e\\
    @dispatched = ();
    $p          = make_parser();
    set_handler( $p,
        { HandleOsc => sub ( $cmd, $data ) { push @dispatched, $data }, HandleEsc => sub ($cmd) { push @dispatched, { esc => $cmd } }, } );
    ...
    # -- utf8: \e]2;echo '¯\_(ツ)_/¯' && sleep 1\e\\  (ST = 0x9C)
    @dispatched = ();
    $p          = make_parser();
    set_handler( $p, { HandleOsc => sub ( $cmd, $data ) { push @dispatched, $data }, } );
    ...
    # -- string_terminator: \e]2;\xe6\x9c\xab\e\\  (partial UTF-8 interrupted)
    @dispatched = ();
    $p          = make_parser();
    set_handler( $p,
        { HandleOsc => sub ( $cmd, $data ) { push @dispatched, $data }, HandleEsc => sub ($cmd) { push @dispatched, { esc => $cmd } }, } );
    ...
    # -- exceed_max_buffer_size: data truncated to buffer
    @dispatched = ();
    $p          = new_parser();
    set_params_size( $p, 16 );
    set_data_size( $p, $max_buf );
    ...
    # -- title_empty_params_esc: \e]0;abc\e\\\e];;;...\a
    @dispatched = ();
    $p          = make_parser();
    set_handler( $p,
        { HandleOsc => sub ( $cmd, $data ) { push @dispatched, $data }, HandleEsc => sub ($cmd) { push @dispatched, { esc => $cmd } }, } );
